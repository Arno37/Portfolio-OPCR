...
const techIcons = [
  ...
  { fa: faDocker, color: "#2496ED", label: "Docker" },
  { fa: faDatabase, color: "#336791", label: "PostgreSQL" },
  { fa: faBrain, color: "#FF6B6B", label: "LangChain" },
  { fa: faGitlab, color: "#FC6D26", label: "GitLab CI/CD" },
];
      ...
      >
        ...
        <Container className="text-center">
          ...
          <div className="technology-icons fade-in-up" style={{
            ...
            {techIcons.map(({ icon, fa, color, label }) => (
              <div key={label} className="tech-icon glass-card" style={{ padding: '1.5rem', minWidth: '120px' }}>
                {fa
                  ? <FontAwesomeIcon icon={fa} size="3x" color={color} />
                  : icon
                }
                <div style={{ marginTop: '10px', fontSize: '1em', fontWeight: 500 }}>{label}</div>
              </div>
            ))}
          </div>
        </Container>
